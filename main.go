...
func contribute(w http.ResponseWriter, r *http.Request) {
	...
	w.Header().Set("Content-Type", "application/json")
	rows, err := database.Query(fmt.Sprintf("INSERT INTO %s VALUES (0, '%s', '%s', '%s', '%s', '%s', %v)", topic, question, option1, option2, option3, option4, answer))
	defer rows.Close()
	if err != nil {
		data := Result{false, "Error!"}
		js, _ := json.Marshal(data)
		w.Write(js)
		return
	}
	...
	data := Result{true, "Thanks for contributing"}
	js, _ := json.Marshal(data)
	w.Write(js)
}
